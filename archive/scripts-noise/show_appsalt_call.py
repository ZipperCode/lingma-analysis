"""Show exact context around getAppSalt call at 0x88114f"""
import struct
import capstone
...
def main():
    ...
    call_rva = 0x88114f
    call_addr = IMAGE_BASE + call_rva
    call_off = rva_to_file(call_rva, sections)

    print(f"getAppSalt call at RVA 0x{call_rva:x}, file offset 0x{call_off:x}")
    print(f"Call bytes: {data[call_off:call_off+5].hex(' ')}")
    print()

    # Show 10 instructions before and 20 after
    all_insns = []
    ...
    if call_idx is None:
        print("Call not found in instruction list!")
        return

    start = max(0, call_idx - 10)
    end = min(len(all_insns), call_idx + 25)

    print(f"Instructions around getAppSalt call:\n")
    ...
    for i in range(start, end):
        ...
        if i == call_idx:
            ...
            marker = "  << right after getAppSalt returns"

        # Show the raw bytes
        hex_bytes = insn.bytes[:15].hex(' ')
        ...
        print(f"  {i:3d} 0x{insn.address:010x} [{hex_bytes:30s}] {insn.mnemonic:8s} {insn.op_str:40s}{extra}{marker}")
