"""
Frida v5: Stalker-based tracing (no code patching)
Stalker copies and instruments code blocks without modifying originals,
which should avoid Go GC's runtime.scanstack crash.
"""
import frida
import time
import subprocess
...
def load_js():
    with open(os.path.join(SCRIPT_DIR, 'hook_stalker.js'), 'r', encoding='utf-8') as f:
        return f.read()
...
def main():
    ...
    print("Loading Stalker hook script...")
    hook_script = load_js()
    ...
    def on_message(msg, data):
        ...
        if isinstance(payload, dict):
            ...
            if msg_type == 'log':
                ...
                print("[STALKER] {}".format(payload.get('msg', '')))
                call_count[0] += 1
            elif msg_type == 'error':
    ...
    script.load()
    print("Hook loaded, waiting 2s...")
    time.sleep(2)

    # Try RPC to trigger getAppSalt
    ...
    try:
        ...
        print("Connected to Lingma pipe!")
        ...
        print("Stalker captures so far: {}".format(call_count[0]))
        for i in range(30):
            time.sleep(1)
            if i % 10 == 0:
                print("  {}s elapsed, captures: {}".format(i, call_count[0]))
