...
> 历史说明（2026-04-25）：
> 这份报告对应的是“远端直连还未被当前仓库脚本实测打通”的阶段。
> 其中认证/签名证据仍然有用，但当前总体状态请以 `docs/lingma-analysis-overview.md` 和 `docs/remote-api-direct-connection.md` 为准。

> 状态说明：这是一份早期阶段报告，里面关于 `machine token`、PKCE 和“无法进程外构造认证”的判断，已经被后续 `docs/lingma-analysis-endpoint-auth.md`、`docs/lingma-analysis-request-flow-pruned.md` 与 `docs/continue-analysis-from-current-state.md` 部分覆盖或修正。继续分析时请把它只当历史证据，不要把这里的“最终”当成当前结论。
...
**注意**: 这是 AI agent 的工具提示中的调试信息，不是实际的服务端验证逻辑。
