...
> 历史说明（2026-04-25）：
> 这份计划形成于“主线仍集中在 getAppSalt / Frida / 本地签名触发”的阶段。
> 现在更高优先级的问题已经转向 `agent_chat_generation` 的完整 body 构造，请不要把本文件继续当作当前唯一待办列表。

> 状态说明：这份“下一步”来自早期 `getAppSalt/Frida` 主导阶段，当前不再适合作为默认行动计划。继续推进时，请优先参考 `docs/continue-analysis-from-current-state.md` 中围绕 `37010`、离进程 GET 和 `Encode=1` POST body 的主线。
