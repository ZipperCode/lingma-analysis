...
> 状态说明：这份文档记录的是早期 `getAppSalt` 试验阶段的判断，很多结论后来已经被更完整的证据链替换。继续推进时，它只适合作为历史背景；当前更推荐优先看 `tools/getappsalt_analysis_v2.md`、`docs/lingma-analysis-endpoint-auth.md` 和 `docs/continue-analysis-from-current-state.md`。
